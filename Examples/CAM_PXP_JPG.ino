...
//  First Test for JPG compression
//  The savejpgA version is for testing direct conversion from camera YUV data
//  MJB  12/22/2020
...
void setup() {
  ...
  CMSI(NULL);
  JPGinit_all();
  Serial.println("JPG Tables initialized.");
}
...
void CMJP(void *cp) {
  elapsedMillis etm;
  uint32_t endtime;
  toutmode cmode = OV7670.OutMode();
  etm = 0;

  if (cmode == RGB565) SaveJpgFile((uint8_t *)rgb8, cmode, 640, 480, NewFileName());
  if (cmode == YUV422) SaveJpgFile((uint8_t *)fcaptbuff, cmode, 640, 480, NewFileName());

  endtime = etm;
  Serial.printf("Converting and saving JPG file took %u mSec\n", endtime);
}
...
void CMGP(void *cp) {
  ...
  *tsptr = lasttime;
  ...
  while (!pxp.Done()) {};
  pxp.Stop();
  Serial.printf("RGB888 image stored at %p\n", rgb8);
}
...
void InitPXP_Image( timagetype  citype, toutmode omode) {
  ...
  if (citype == QVGA) {
    pxp.begin((uint16_t *)fcaptbuff, (uint16_t *)rgb8,  320, 240, 640, 480 );
    // now set scaling for 2x H and V
    pxp.SetScale(2.0);
  }
  if (citype == VGA) {
    pxp.begin((uint16_t *)fcaptbuff, (uint16_t *)rgb8,  640, 480, 640, 480 );
    pxp.SetScale(1.0);
  }
  if (citype == QQVGA) {
    pxp.begin((uint16_t *)fcaptbuff, (uint16_t *)rgb8,  160, 120,  640, 480 );
    // now set scaling to multiply by 4X H and V
    pxp.SetScale(4.0);
  }
